...
import { possibleWord } from '../../words'
...
function wordIsInDictionary({ attempts, position }) {
  ...
  return possibleWord.words.some(word => word.toUpperCase() === guess)
}
...
function Board() {
  const [{ word, attempts, position, error }, setGameState] =
  ...
  function guessWord() {
    setGameState(gameState => {
      if (wordIsNotComplete(gameState)) {
        return { ...gameState, error: 'Palavra incompleta.' }
      }
  ...
  function updateStatusKey(newPosition) {
    setGameState(gameState => {
      gameState.attempts[newPosition.attempt][newPosition.char].status =
        'at-location'
    ...
    document.getElementById(
      `char-${newPosition.attempt}-${newPosition.char}`
    ).className = 'char at-location'
  }
          ...
          >
            {attempt.map((char, charIndex) => (
              <span
                ...
                onDoubleClick={() => {
                  updateStatusKey({ attempt: attemptIndex, char: charIndex })
                }}
              >
                {char.value}
              </span>
